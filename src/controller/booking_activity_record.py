...
class BookingActivityRecord:
    ...
    def __init__(self, booking_id: str, room_id: str,
        ...
        self.__date       = date        # dd/mm/yyyy
        self.__time       = time        # HH:MM start time
    ...
    def as_datetime(self) -> datetime:
        """
        Return a datetime representing when this booking started.
        Used as the sort key for chronological ordering.
        Falls back to datetime.min if the stored strings are malformed.
        """
        try:
            return datetime.strptime(
                f"{self.__date} {self.__time}", self._DATETIME_FORMAT
            )
        except ValueError:
            return datetime.min
